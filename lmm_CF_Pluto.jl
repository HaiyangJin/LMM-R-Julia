...
# ╔═╡ 694f0884-0f27-11ec-30eb-e148a7ce360c
begin
	using DataFrames
...
# ╔═╡ c14a163b-01f3-43e9-9376-2e62fa896751
PlutoUI.TableOfContents(title="Mixed Models in Julia")

# ╔═╡ f5af0f2b-51e2-4fc5-9d97-ab9d88eced5f
md"""
# Load data
"""

# ╔═╡ 69fe2a68-263f-43ef-b3a6-3003e6592f85
# dat = rcopy(R"readRDS('./data/EmotikonSubset.rds')");
# df_cf = CSV.read("data/example_data.csv", DataFrame) 
df_cf = DataFrame(Arrow.Table("data/example_sd.arrow"))

# ╔═╡ 1369b4c1-d98e-4c79-96d6-9829801f23b8
contr = merge(
	Dict(:Participant => Grouping()),
	# Dict(x => Grouping() for x in (:Participant, :Item)), 
	Dict(:Condition => SeqDiffCoding(levels = ["CFS", "monocular"])),
	Dict(:Congruency => SeqDiffCoding(levels = ["incongruent", "congruent"])),
	Dict(:Alignment => SeqDiffCoding(levels = ["misaligned", "aligned"])),
	Dict(:answer => SeqDiffCoding(levels = ["different", "same"]))
)

# ╔═╡ 99a6a862-d59f-43db-86a7-c8bc1f098ca0
md"## Make variable (columns) for random effects"

# ╔═╡ 95d1ca6f-adcd-484c-a9cc-b52ca58aefc6
...
# ╔═╡ 9bfa7fa5-d98b-46ea-8105-87919ba528ca
md"""
# Behavioral responses
With signal detectin theory -- GLMM with bernoulli and probit link.
## Fitting the model
"""

# ╔═╡ 97d54001-bbc5-4178-986f-0326a13e43d0
md"### (an equivalent) ZCP model"

# ╔═╡ bc8437f1-a5bf-4b3a-8beb-51326ee3aa6f
begin
...
# ╔═╡ 0bd18431-f206-4c4d-b470-cee0c9e62a5d
issingular(glmm_resp_zcp_)

# ╔═╡ 84d67340-4cef-47cf-98d0-31d23cd1be98
md"### ZCP model"

# ╔═╡ 56de6608-4070-4382-815d-e47fbaac7f5d
begin
...
# ╔═╡ e064abcc-9fed-44b2-8d60-77645a90d1da
issingular(glmm_resp_zcp)

# ╔═╡ 6f91bc64-506e-4e9a-bcf9-4add292bf118
MixedModels.likelihoodratiotest(glmm_resp_zcp, glmm_resp_zcp_)

# ╔═╡ e30a5e3e-7bfd-4f1d-952b-58c1b46e1ed1
# MixedModels.rePCA(glmm_resp_zcp)
glmm_resp_zcp.rePCA

# ╔═╡ 8b284657-68ed-4868-a345-b79eceec5abf
VarCorr(glmm_resp_zcp)

# ╔═╡ a612cc7c-3937-4c9e-93ce-2f9b9638972e
md"### Reduced model"

# ╔═╡ 570d3463-8ff2-44f4-9171-aa03e93306d5
begin
	f_resp_rdc = @formula(resp ~ Condition * Congruency * Alignment * answer +
		zerocorr(cond_main + cong_main + ali_main + ans_main +
			cond_ali + cong_ali + cong_ans + # ali_ans + cond_ans + cond_cong + 
			cond_cong_ali + cong_ali_ans # + cond_ali_ans + cond_cong_ans + 
...
# ╔═╡ 0e643f4b-af86-47e1-b1af-05805e2d8f35
issingular(glmm_resp_rdc)

# ╔═╡ de3e4229-a7da-47d3-8b2b-78d2ab198196
md"### Extended model"

# ╔═╡ e3ee6493-74b8-4a95-8523-55b4919046b1
begin
...
# ╔═╡ abf025dd-118e-464f-bb72-ab28eaa116a7
issingular(glmm_resp_etd)

# ╔═╡ 5ea0a317-ed27-4ac2-a6f7-43adad2184c0
glmm_resp_etd.rePCA

# ╔═╡ eb5c232d-1e40-4038-bdd3-80bf8e1b0874
VarCorr(glmm_resp_etd)

# ╔═╡ 67406e09-6ffe-4f00-bb8f-8e05f5c8ef13
md"### Extended model 2"

# ╔═╡ 17266250-3568-4585-9ff9-efdd150bb7c8
begin
	f_resp_etd2 = @formula(resp ~ Condition * Congruency * Alignment * answer +
		(ans_main + # cong_main + ali_main + cond_main + 
			cong_ali + # ali_ans + cond_ans + cond_cong + cong_ans + cond_ali + 
			cond_cong_ali + cong_ali_ans # + cond_ali_ans + cond_cong_ans + 
			| Participant)) # cond_cong_ali_ans
...
# ╔═╡ cf5a1641-c0f2-4b5e-a6de-c2bd943ec674
issingular(glmm_resp_etd2)

# ╔═╡ 7a7e2369-da92-4a0a-8b1d-c7cfeb52fade
glmm_resp_etd2.rePCA

# ╔═╡ e925d6df-0d1d-4b6f-9431-6a0a7db3a537
VarCorr(glmm_resp_etd2)

# ╔═╡ 1bf3c74f-8deb-420b-88a4-ebd18abf17ac
md"### Extended model 3"

# ╔═╡ 1def6e0e-7f76-45cc-b425-71b7eae77968
begin
...
# ╔═╡ e196247b-c03c-479d-af88-03b4d09c732c
issingular(glmm_resp_etd3)

# ╔═╡ 6b5e6192-77c3-4326-9979-419c0c057952
glmm_resp_etd3.rePCA

# ╔═╡ c35a1d8c-1057-4424-ba54-057dc3ba315f
md"### Optimal model"

# ╔═╡ 0e9e9d0e-d842-4cac-9ef4-f39629e2c4f9
MixedModels.likelihoodratiotest(glmm_resp_etd3, glmm_resp_rdc)

# ╔═╡ 55f7304d-444b-4d3b-9bf9-d60aa81d493a
let 
...
# ╔═╡ b3f0d28f-6035-47f4-8214-5568bee134a3
glmm_resp_opt = glmm_resp_rdc

# ╔═╡ d4f58c05-be90-4b76-a274-0209fbeef86d
issingular(glmm_resp_opt)

# ╔═╡ c694ac62-4ecf-4d58-98da-f6d7f85391c8
md"## Bootstrap"

# ╔═╡ 6f486b89-af9a-416f-8bb7-a18d00ab5ab6
bs_resp1 = parametricbootstrap(MersenneTwister(42), 2, glmm_resp_opt);

# ╔═╡ 980b8efc-467e-4b94-9757-63dd28632128
md"# Response times (lognormal)"

# ╔═╡ fd24332e-98ba-4c5d-ad3e-5df580dff53a
md"## Fitting models"

# ╔═╡ dafc181e-a4ba-47b2-b80c-4e917c2eadac
md"### (An equivalent) ZCP model"

# ╔═╡ e599f112-78f3-4cf3-ab65-7a4ed661caa4
begin
	f_rt_zcp_ = @formula(log(RT) ~ Condition * Congruency * Alignment +
...
# ╔═╡ 2c92b469-aedf-417b-a772-e1d8128daa73
md"### ZCP model"

# ╔═╡ e4193099-afa6-43af-8e31-d4fab86a6b0c
begin
	f_rt_zcp = @formula(log(RT) ~ Condition * Congruency * Alignment +
...
# ╔═╡ 3f6a662f-57ed-4c17-9a66-5977512729db
issingular(lmm_rt_zcp)

# ╔═╡ 9b241e5b-d855-44b6-8701-1609335b6518
lmm_rt_zcp.rePCA

# ╔═╡ 81dc66ff-570a-4163-8d2c-92e2af2f884e
VarCorr(lmm_rt_zcp)

# ╔═╡ a82c7416-d498-4b6b-86d4-e0dd29ee7660
md"### Reduced model"

# ╔═╡ 05326917-493d-475f-a8f9-a681b414153c
begin
	f_rt_rdc = @formula(log(RT) ~ Condition * Congruency * Alignment +
		zerocorr(cond_main + # cong_main + ali_main + 
...
# ╔═╡ 27626c1a-2c70-4bd9-b279-0d04908cff42
issingular(lmm_rt_rdc)

# ╔═╡ cfca0809-ad17-4d60-a2a2-df7f87466e7a
md"### Extended model"

# ╔═╡ 056a1b32-4b5d-4b30-8e1c-16e47f65ab1a
begin
	f_rt_etd = @formula(log(RT) ~ Condition * Congruency * Alignment +
...
# ╔═╡ 5a46ed4c-9f3a-4664-ad29-398bbe8bc235
issingular(lmm_rt_etd)

# ╔═╡ 0c1fd72b-20f3-4530-b8eb-e37d05dd4587
lmm_rt_etd.rePCA

# ╔═╡ 3ff07fea-1665-4385-a891-cbd23a434f48
VarCorr(lmm_rt_etd)

# ╔═╡ e381375a-58c3-45dc-bef4-764004616c4c
md"### Optimal model"

# ╔═╡ f1bf7920-f62c-4a99-b335-94742e50ab69
MixedModels.likelihoodratiotest(lmm_rt_etd, lmm_rt_rdc)

# ╔═╡ 0eb2dabb-ec77-4fbd-ba37-9f6d8e613721
let 
...
# ╔═╡ 26b0569c-3e54-495c-ada1-cbc65df9c81d
md"""
The reduced model `lmm_rt_rdc` is used as the optimal model due to smaller AIC. 
"""

# ╔═╡ aea35b04-cc42-49c1-847a-6e667486e23b
lmm_rt_opt = lmm_rt_rdc

# ╔═╡ ac305e76-9695-4b39-832c-7a3c27e9d2b8
VarCorr(lmm_rt_opt)

# ╔═╡ 9b32669a-83c1-4f5f-b31f-f4b3bbb32da9
shrinkageplot(lmm_rt_opt)

# ╔═╡ 2c0e75ec-5db3-4e51-9b98-78ab142ac521
md"## Bootstrap"

# ╔═╡ 6590d265-65ac-4fbe-bb90-a333dbcf360d
bs_rt = parametricbootstrap(MersenneTwister(42), 2000, lmm_rt_opt)

# ╔═╡ 1724edba-0726-4869-b02f-14603776063c
# begin
# 	samples = DataFrame(bs_rt.allpars);
# 	Arrow.write("data/samples.arrow", samples);
# end

# ╔═╡ 043ae5cf-9c80-454b-931f-c9b65041ecab
DataFrame(shortestcovint(bs_rt))

# ╔═╡ 2721606d-8809-435c-8e17-579988930736
# ridgeplot(bs_rt)

# ╔═╡ ed44e084-6d56-48f1-b0ac-e0e92691058f
md"## Effects of interest"

# ╔═╡ fec57f23-14bf-467c-95d9-bd8b671764ac
beta_sample = @chain DataFrame(bs_rt.β) begin
	@transform(:coefname = string.(:coefname))
	@transform(:coefname = 
	...
	unstack(:coefname, :β)
	@transform(
		:congruency_mono = :cong_main * 2 + :cond_cong,
		:congruency_cfs = :cong_main * 2 - :cond_cong,
		:cf_mono = :cond_ali * 2 + :cond_cong_ali, # cf for monocular
		:cf_cfs = :cond_ali * 2 - :cond_cong_ali,
		:cf_diff = :cond_cong_ali) # and this
	@select(:congruency_mono, :congruency_cfs, :cf_mono, :cf_cfs, :cf_diff)
	stack()
end

# ╔═╡ 18fca8e4-f752-4391-8547-5528aee1db9c
begin
	f_rt_ = @formula(RT ~ Condition * Congruency * Alignment +
		(1
			| Participant)) # cond_cong_ali 
...
# ╔═╡ 64fbed61-8e42-41d6-b614-e41f168fee3c
begin
	design = Dict(
	...
	preds = effects(design, lmm_rt_)
end

# ╔═╡ 482ee3d7-a47e-4ec5-bcde-0732cd9519a6
lmm_rt_opt

# ╔═╡ 8e19b0bf-ecce-44b6-bb27-87618aa2d5bf
data(bs_rt.β) * mapping(:β; layout = :coefname) * AOG.density() |> draw

# ╔═╡ fe1c6694-1595-4467-b0fc-268715689817
bs_rt.allpars

# ╔═╡ 63b6b8af-cfd5-41d0-b09f-3da42bc4b69e
data(filter(:type => ==("ρ"), DataFrame(bs_rt.allpars))) * mapping(:value, layout = :names) * AOG.density() |> draw
